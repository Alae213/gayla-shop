      ...
      {columns.map(column => {
        ...
        return (
          <section 
            ...
            aria-labelledby={`kanban-col-heading-${column.id}`}
          >
            {/* Column Header */}
            <div className="group relative flex items-center justify-between mb-4 px-2 py-1">
              <div className="flex items-center gap-3">
                <TrackingCheckbox 
                  ...
                  aria-label={isAllSelected ? `Deselect all ${column.title} orders` : `Select all ${column.title} orders`}
                  className={isAllSelected || hasSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100 focus-visible:opacity-100"}
                />
                <div className="flex items-center gap-2">
                  <h3 id={`kanban-col-heading-${column.id}`} className="text-[15px] font-semibold text-[#3A3A3A] lowercase m-0">
                    {column.title}
                  </h3>
                  <span 
                    className="flex items-center justify-center bg-[#ECECEC] text-[#AAAAAA] text-[12px] font-medium h-6 min-w-[24px] px-1.5 rounded-full"
