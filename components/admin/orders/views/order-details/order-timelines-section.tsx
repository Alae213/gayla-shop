...
import { Clock, ChevronDown, ChevronUp, PhoneCall, PhoneMissed, PhoneForwarded, PhoneOff, CheckCircle2, Package, Truck, X, ShieldOff } from "lucide-react";

type CallOutcome = "answered" | "no answer" | "wrong number" | "refused";

const OUTCOME_META: Record<CallOutcome, { label: string; icon: React.ElementType; color: string; bg: string }> = {
...
export function OrderTimelinesSection({ callLog, statusHistory, courierTrackingId, showCallLog, showStatusHistory }: OrderTimelinesSectionProps) {
  return (
    <>
      {showCallLog && callLog.length > 0 && <CallLogHistory callLog={callLog} />}
      {showStatusHistory && statusHistory.length > 0 && (
        <section className="mt-8 pt-6 border-t border-[#ECECEC]" aria-labelledby="timeline-heading">
          <h3 id="timeline-heading" className="text-[14px] font-semibold text-[#3A3A3A] mb-4 flex items-center gap-2">
            <Clock className="w-4 h-4 text-[#AAAAAA]" /> Order Timeline
          </h3>
          <OrderTimeline entries={statusHistory} />
        </section>
      )}
      {courierTrackingId && (
        <a href={`https://yalidin.com/track/${courierTrackingId}`} target="_blank" rel="noopener noreferrer" className="text-[13px] text-blue-600 underline mt-4 inline-block">
          Track with Yalidin →
        </a>
      )}
    </>
...
function CallLogHistory({ callLog }: { callLog: Array<{ timestamp: number; outcome: CallOutcome; note?: string }> }) {
  ...
  return (
    <section className="mt-8 pt-6 border-t border-[#ECECEC]" aria-labelledby="call-log-heading">
      <button onClick={() => setExpanded(p => !p)} className="flex items-center justify-between w-full text-left group" aria-expanded={expanded}>
        <h3 id="call-log-heading" className="text-[14px] font-semibold text-[#3A3A3A] flex items-center gap-2">
          ...
          Call History
          <span className="ml-1 text-[12px] font-normal text-[#AAAAAA] bg-[#F5F5F5] px-2 py-0.5 rounded-full">{callLog.length} attempt{callLog.length !== 1 ? "s" : ""}</span>
        </h3>
            ...
            return (
              <div key={i} className="relative pl-8 animate-in fade-in slide-in-from-left-2 duration-300">
                ...
                <div className="flex flex-col">
                  ...
                  {entry.note && <p className="mt-1 text-[13px] text-[#555555] bg-white border border-[#ECECEC] rounded-lg p-2 italic leading-relaxed">&ldquo;{entry.note}&rdquo;</p>}
                  <span className="mt-1 text-[11px] text-[#AAAAAA] uppercase tracking-wider font-medium">{format(entry.timestamp, "MMM d, HH:mm")}</span>
                </div>
              </div>
            );
          })}
...
function OrderTimeline({ entries }: { entries: Array<{ status: string; timestamp: number; reason?: string }> }) {
  const statusIcon: Record<string, React.ElementType> = {
    new: PhoneCall, confirmed: CheckCircle2, packaged: Package, shipped: Truck, canceled: X, blocked: ShieldOff, hold: PhoneForwarded,
  };
  return (
