...
import { Upload, Sparkles, Check, X, ImageOff, Loader2 } from "lucide-react";
...
function HeroPreviewCard({
  title, subtitle, ctaText, bgUrl,
}: {
  title: string; subtitle: string; ctaText: string; bgUrl: string | null;
}) {
  return (
    <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-gray-900 shadow-lg">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5">
        ...
        {subtitle && <p className="text-gray-300 text-sm mt-1 drop-shadow">{subtitle}</p>}
        {ctaText && (
...
export function HeroEditor({ siteContent, onSave, onDirtyChange }: HeroEditorProps) {
  ...
  const [isFlushing,       setIsFlushing]       = useState(false); // M3 Task 3.4
  const [showRemoveDialog, setShowRemoveDialog] = useState(false);

  const fileInputRef  = useRef<HTMLInputElement>(null);
  const debounceTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  // Store latest field values for the flush path
  const latestFields  = useRef({ title: "", subtitle: "", ctaText: "Shop Now" });
  ...
  useEffect(() => {
    if (siteContent) {
      const t = siteContent.heroTitle    || "";
      const s = siteContent.heroSubtitle || "";
      const c = siteContent.heroCtaText  || "Shop Now";
      setHeroTitle(t);
      setHeroSubtitle(s);
      setHeroCtaText(c);
      latestFields.current = { title: t, subtitle: s, ctaText: c };
    }
  }, [siteContent]);
  ...
  // Debounced save — also keeps latestFields in sync for the flush path
  const debouncedSave = useCallback(
    (title: string, subtitle: string, ctaText: string) => {
      latestFields.current = { title, subtitle, ctaText };
  ...
  /**
   * M3 Task 3.4 — Done button handler.
   * If a debounce is pending (saveStatus==='saving'), cancel the timer,
   * fire the save immediately, wait for it, THEN exit edit mode.
   * This prevents any data loss on fast Done clicks.
   */
  const handleDone = useCallback(async () => {
    if (saveStatus === "saving" && debounceTimer.current) {
      clearTimeout(debounceTimer.current);
      debounceTimer.current = null;
      setIsFlushing(true);
      try {
        const { title, subtitle, ctaText } = latestFields.current;
        await updateSiteContent({ heroTitle: title, heroSubtitle: subtitle, heroCtaText: ctaText });
        setSaveStatus("saved");
        onSave();
        setTimeout(() => setSaveStatus("idle"), 5000);
  ...
  }, [saveStatus, updateSiteContent, onSave]);

  // Task 2.4 — XHR upload with progress
  ...
  return (
    <div id="section-hero" className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* M3 Task 3.4 — Done flushes in-flight save */}
          <Button
            variant={editingHero ? "default" : "outline"}
            onClick={editingHero ? handleDone : () => setEditingHero(true)}
            size="sm"
            disabled={isFlushing}
          >
            {isFlushing ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div>
              ...
              <Input id="heroTitle" value={heroTitle}
                onChange={(e) => { const v = e.target.value; setHeroTitle(v); debouncedSave(v, heroSubtitle, heroCtaText); }}
                placeholder="Welcome to Gayla" className="mt-2" />
            </div>
            <div>
              <Label htmlFor="heroSubtitle">Hero Subtitle</Label>
              <Input id="heroSubtitle" value={heroSubtitle}
                onChange={(e) => { const v = e.target.value; setHeroSubtitle(v); debouncedSave(heroTitle, v, heroCtaText); }}
                placeholder="Discover premium streetwear" className="mt-2" />
            </div>
            <div>
              <Label htmlFor="heroCtaText">Call-to-Action Button Text</Label>
              <Input id="heroCtaText" value={heroCtaText}
                onChange={(e) => { const v = e.target.value; setHeroCtaText(v); debouncedSave(heroTitle, heroSubtitle, v); }}
                placeholder="Shop Now" className="mt-2" />
            </div>
            {/* Image controls */}
            <div>
              ...
              <div className="mt-3 space-y-3">
                {hasImage && (
                  <div className="relative inline-block">
                    <div className="relative h-[120px] w-[200px] rounded-lg overflow-hidden border border-gray-200">
                      <Image src={resolvedBgUrl!} alt="Hero background" fill className="object-cover" />
                      {uploadProgress !== null && (
                    ...
                    {uploadProgress === null && (
                      <button type="button" onClick={() => setShowRemoveDialog(true)}
                ...
                <div>
                  <input ref={fileInputRef} type="file" accept="image/*" onChange={handleHeroBackgroundUpload} className="hidden" />
                  <Button variant="outline" onClick={() => fileInputRef.current?.click()} disabled={uploadProgress !== null} className="w-full">
                    {uploadProgress !== null
          ...
          <div className="space-y-2">
            ...
            <HeroPreviewCard title={heroTitle} subtitle={heroSubtitle} ctaText={heroCtaText} bgUrl={resolvedBgUrl} />
          </div>
        </div>
      ) : (
        /* Preview-only mode */
      ...
      <AlertDialog open={showRemoveDialog} onOpenChange={setShowRemoveDialog}>
        <AlertDialogContent className="max-w-sm">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleRemoveImage} className="bg-red-600 hover:bg-red-700 text-white">
              Remove image
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
