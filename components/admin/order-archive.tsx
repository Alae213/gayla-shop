...
export function OrderArchive({ orders, stats, onOrderClick }: OrderArchiveProps) {
  ...
  // Fix 3: checkbox selection state
  ...
  const purgeByIds      = useMutation(api.orders.purgeByIds);
  ...
  const daysUntilPurge = preview?.oldestCreationTime
    ? Math.max(0, Math.ceil((preview.oldestCreationTime + SIXTY_DAYS_MS - now) / (24 * 60 * 60 * 1000)))
    : null;

  // Fix 3: clear selection when filter changes
  const handleFilterChange = (val: ArchiveFilter) => {
    setFilter(val);
    setSelectedIds(new Set());
  };

  // E3: Force Clean (all orders older than 60 days)
  const handleForceClean = async () => {
    ...
    try {
      ...
      setSelectedIds(new Set()); // clear selection after purge
    } catch (e: any) {
  ...
  // Fix 3: Delete selected IDs
  const handleDeleteSelected = async () => {
    if (selectedIds.size === 0) return;
    setIsDeleting(true);
    try {
      const ids = Array.from(selectedIds) as Id<"orders">[];
      const result = await purgeByIds({ ids });
      toast.success(`🗑️ Deleted ${result.deleted} selected order${result.deleted !== 1 ? "s" : ""}.`);
      setSelectedIds(new Set());
    } catch (e: any) {
      toast.error(e.message || "Failed to delete selected orders");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-6">
              ...
              {oldestDate ? (
                <>
                  ...
                  <p className={`text-xs mt-0.5 ${
                    (daysUntilPurge ?? 999) === 0 ? "text-red-500 font-medium"
                    : (daysUntilPurge ?? 999) <= 7  ? "text-amber-500"
                    : "text-gray-400"
                  }`}>
                    {daysUntilPurge === 0 ? "⚠️ Eligible for purge now" : `Auto-purge in ${daysUntilPurge}d`}
                  </p>
                </>
              ) : (
          ...
          <div className="ml-auto flex items-center gap-3 flex-wrap">
            ...
            {!showConfirm ? (
              ...
              >
                <Trash2 className="h-3.5 w-3.5" /> Force Clean
              </Button>
            ) : (
      ...
      {/* Fix 3: Floating action bar — visible when rows are selected */}
      {selectedIds.size > 0 && (
